...
[![CRAN Version](http://www.r-pkg.org/badges/version/BGGMmod)](https://cran.r-project.org/package=BGGMmod)
[![Downloads](https://cranlogs.r-pkg.org/badges/BGGMmod)](https://cran.r-project.org/package=BGGMmod)
[![Build Status](https://travis-ci.com/donaldRwilliams/BGGMmod.svg?branch=master)](https://travis-ci.com/donaldRwilliams/BGGMmod)
...
The following includes brief examples for *some* of the methods in **BGGMmod**.
...
#### Confirmatory
A central contribution of **BGGMmod** is confirmatory hypothesis testing of (in)equality constraints [@Hoijtink2011]. By this we are referring to testing expectations, as opposed to feeding the data to, say, `estimate`, and seeing what happens to emerge. 
...
This demonstrates that all the `plot()` functions in **BGGMmod** return `ggplot` objects that can be further customized. Note that **BGGMmod** is not focused on making publication ready plots. Typically the bare minimum is provided that can then be honed in.
...
This next example is a new feature in **BGGMmod** (`2.0.0`), that allows for comparing GGMs any way the user wants. All that is required is to (1) decide on a test-statistic and (2) write a custom function. 
...
#### Confirmatory (groups)
A central contribution of **BGGMmod** is confirmatory hypothesis testing of (in)equality constraints [@Hoijtink2011],
...
In this example, predictability is computed for each node in the network [see here for rationale @haslbeck2018well]. Currently **BGGMmod** computes Bayesian variance explained for all data types [@gelman_r2_2019].  
...
A new feature to **BGGMmod** allows for computing user defined network statistics, given a partial correlation or
...
Note that **BGGMmod** does not use the customary `model.matrix` formulation. This is for good reason, as 
each variable in the GGM does not need to be written out. Here we effectively "tricked" **BGGMmod** to 
...
**BGGMmod** was built specifically for social-behavioral scientists. Of course, the methods
...
Bug reports and feature requests can be made by opening an issue on [Github](https://github.com/donaldRwilliams/BGGMmod/issues). To contribute towards
the development of **BGGMmod**, you can start a branch with a pull request and we can 
