...
# Introduction
This vignette describes how to control for variables. This is a new feature to **BGGMmod** (version `2.0.0`).

# Example 1: Multivariate Regression
When controlling for variables, a multivariate regression is fitted in **BGGMmod**. In fact, a GGM can be understood as a multivariate regression with intercepts only models for the predictors.

## Notes about Implementation
**BGGMmod** does not use the typical approach for multivariate regression in `R`. This avoids having to 
write out each outcome variable, of which there are typically many in a GGM. In **BGGMmod**, it is assumed
...
(six variables instead of the desired 5). This is because all variables not included in `formula` are included in the GGM. This was adopted in **BGGMmod** to save the user from having to write out each outcome.  
...
The above data is ordinal. In this case, it is possible to fit a multivariate probit model. This is also the approach for binary data in **BGGMmod**. This is implemented with 
...
# Note
It is possible to control for variable with all methods in **BGGMmod**, including when comparing groups, Bayesian hypothesis testing, etc.
