...
To date, most `R` packages provide a few ways to compare groups, including **BGGMmod** (version `1.0.0`). 
In version `2.0.0`, however, **BGGMmod** includes a new feature for the function `ggm_compare_ppc` that enables 
...
In **BGGMmod**, the default is to compare the groups with respect to (symmetric) Kullback-Leibler 
...
ppc <- BGGMmod::ggm_compare_ppc(Yg1, Yg2, 
                       FUN = f, 
                       custom_obs = obs, 
                       iter = 1000, 
                       loss = FALSE)
...
f <- function(Yg1, Yg2){
  ...
  # fit models
  fit1 <-  BGGMmod::estimate(Yg1, analytic = TRUE)
  fit2 <-  BGGMmod::estimate(Yg2, analytic = TRUE)
  
  # select graphs
  sel1 <- BGGMmod::select(fit1)
  sel2 <- BGGMmod::select(fit2)
...
f <- function(Yg1, Yg2){
  ...
  pcor1 <- BGGMmod::pcor_mat(fit1) 
  pcor2 <- BGGMmod::pcor_mat(fit2)
...
1. Note that only the default in **BGGMmod** have been shown to have nominal error rates. However, there is a proof that suggests the error rate cannot be larger than $2\alpha$ [@meng1994posterior], and, further, a predictive check is typically below $\alpha$ [i.e., a tendency to be conservative, @gelman2013two]. 
