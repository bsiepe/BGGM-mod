...
the comments, it was suggested that **BGGMmod** could be used to test the hypothesis. And it turns
out that **BGGMmod** really shines for testing expectations [see for example @rodriguez2020formalizing]. 
...
implemented by the user. Note that I could certainly implement this in **BGGMmod**, assuming there 
is enough interest. Please make a feature request [here](https://github.com/donaldRwilliams/BGGMmod/issues).
...
\end{align}
$$
Note that the hypothesis is related to the sum of relations, which is readily tested in **BGGMmod**. 
...
\end{align}
$$
where the only difference is $yrep$. See more details [here](https://donaldrwilliams.github.io/BGGMmod/articles/ppc_custom.html).
...
Also, the default in **BGGMmod** does not require a custom function 
...
(although this can be changed to `type = "ordinal"` or `type = "mixed"`; see [here](https://donaldrwilliams.github.io/BGGMmod/reference/ggm_compare_confirm.html))
...
# Conclusion
Three approaches for testing the same hypothesis were demonstrated in this vignette. This highlights that any hypothesis can be tested in **BGGMmod** and in several ways. 
